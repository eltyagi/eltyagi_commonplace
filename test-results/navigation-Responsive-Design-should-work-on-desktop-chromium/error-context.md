# Page snapshot

```yaml
- generic [ref=e5]:
  - generic [ref=e7]:
    - generic [ref=e8]: eltyagi's
    - img "Logo" [ref=e10]
    - generic [ref=e11]: commonplace
  - generic [ref=e12]:
    - generic [ref=e13]: Hi there, I’m Lakshya and welcome to my digital home!
    - text: I am a Solutions Engineer,
    - link "GitHub" [ref=e14] [cursor=pointer]:
      - /url: https://github.com/eltyagi
    - text: . By day, I help CTOs, Engineering Managers and VPs drive engineering productivity. By night, I am building, creating and teaching and more.. There is much to cover, so much to learn so I am happy to have you here.
  - generic [ref=e15]:
    - img "LinkedIn" [ref=e16]
    - img "Medium" [ref=e17]
    - img "Instagram" [ref=e18]
  - generic [ref=e20]:
    - link "Logo" [ref=e22] [cursor=pointer]:
      - /url: /
      - img "Logo" [ref=e23] [cursor=pointer]
    - link "Thoughts" [ref=e24] [cursor=pointer]:
      - /url: /thoughts
    - link "Meditations" [ref=e25] [cursor=pointer]:
      - /url: /meditations
    - link "Me?" [ref=e26] [cursor=pointer]:
      - /url: /about
    - link "Say hi!" [ref=e27] [cursor=pointer]:
      - /url: /contact
```